...
import { generateJWT, verifyToken } from "../util/token.ts";
import { bcryptPass, comparePass } from "../util/crypt.ts";
import  MongooseError  from "mongoose";
import { matric } from "../util/matricule.ts";
    ...
    static async editStudent(req:Request, res:Response){
       ...
       try {
            ...
            if(studentUser &&  id == auth._id){
                if(logo && password){
                    studentUser.updateOne({logo:`${req.protocol}://${req.get('host')}/images/${req.file.filename}`, password:bcryptPass(password), ...body})
                }
                else if(logo){
                    studentUser.updateOne({logo:`${req.protocol}://${req.get('host')}/images/${req.file.filename}`, ...body})
                }
                else if(password){
                    studentUser.updateOne({password:bcryptPass(password), ...body})
                }
                else{
                    studentUser.updateOne({ ...body})
                }
            }
            res.status(200).json({
                status: true,
